    ...
    <div className="flex items-center shrink-0">
      ...
      {!isLast && (
        <div className="mx-1.5">
          {isDelivered ? (
            <div className="w-8 h-0.5 bg-green-400 dark:bg-green-600" />
          ) : isInTransit ? (
            <div className="w-8 border-t-2 border-dashed border-blue-400 dark:border-blue-500" />
          ) : (
...
function RouteCard({
  ...
  onSelectStop: (stop: DeliveryStop, route: DeliveryRoute) => void;
}) {
  ...
  // Payment helper
  const getPaymentStatus = (orderId: string): 'paid' | 'unpaid' =>
    orders.find((o) => o.id === orderId)?.paymentStatus ?? 'unpaid';

  // Payment stats across all stops
  const unpaidCount = route.stops.filter(
    (s) => getPaymentStatus(s.orderId) === 'unpaid'
  ).length;
  const paidCount = totalStops - unpaidCount;

  // Current stop: in_transit stop, or first pending if not delivered
  const currentStop =
    route.stops.find((s) => s.status === 'in_transit') ??
    (route.status !== 'delivered'
      ? route.stops.find((s) => s.status === 'pending')
      : null);

  const currentStopPayment = currentStop
    ? getPaymentStatus(currentStop.orderId)
    : undefined;
    ...
    <AnimatedCard className="overflow-hidden">
      <div className="p-4 sm:p-5 space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          {/* Left: Truck icon + Route ID + Driver info */}
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-sm leading-tight">{route.id}</h3>
        ...
        {/* === 2. Progress Tracker (Horizontal Journey Strip) === */}
          ...
          <div className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/30 px-3 py-1.5">
            ...
            <span className="text-xs font-medium text-blue-700 dark:text-blue-300">
              ...
              {new Date(
                route.rescheduledDate +
                  'T' +
                  (route.rescheduledTime || '08:00')
              ).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
              })}{' '}
              at {route.rescheduledTime || '08:00'}
            </span>
          </div>
        )}

        {/* === 3. Current Stop Section === */}
        {currentStop && (isActive || isPending) && (
          <div className="rounded-lg border border-blue-200 bg-blue-50/50 dark:border-blue-800 dark:bg-blue-950/20 p-3.5 border-l-4 border-l-blue-500 dark:border-l-blue-400">
        ...
        {isDelivered && (
          <div className="rounded-lg border border-green-200 bg-green-50/50 dark:border-green-800 dark:bg-green-950/20 p-3.5 border-l-4 border-l-green-500 dark:border-l-green-400">
        ...
        {/* === 5. All Stops Section (Expandable) === */}
        <div className="rounded-lg border bg-muted/30">
          <button
            type="button"
            className="flex items-center justify-between w-full p-3 hover:bg-muted/50 transition-colors rounded-lg text-left"
            onClick={() => setStopsExpanded(!stopsExpanded)}
          >
            <div className="flex items-center gap-2">
              <MapPin className="h-4 w-4 text-muted-foreground" />
              <span className="text-sm font-medium">All Stops ({totalStops})</span>
              {unpaidCount > 0 && (
                <Badge
                  variant="destructive"
                  className="text-[10px] px-1.5 py-0 h-5"
            ...
            {stopsExpanded && (
              ...
              >
                <div className="border-t px-3 py-2.5 space-y-2.5">
                  {route.stops.map((stop, idx) => {
                    const paymentStatus = getPaymentStatus(stop.orderId);
                    return (
                      <button
                        key={stop.id}
                        type="button"
                        className="flex items-start gap-2.5 w-full text-left hover:bg-muted/50 rounded-md p-1 -m-1 transition-colors"
                        onClick={(e) => {
                          e.stopPropagation();
...
export default function DeliveriesPage() {
  ...
  const handleSelectStop = useCallback(
    (stop: DeliveryStop, route: DeliveryRoute) => {
